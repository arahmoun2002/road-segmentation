...
def get_preprocessed_data(path: str = TRAIN_PATH, val_size: float = 0.2):
    """Load and preprocess the data from the original kaggle competition dataset.
...
def get_deepglobe_data(path: str, N_FILES=1000):
    """Get DeepGlobe data.

    This function takes a path to the directory containing the DeepGlobe dataset and returns preprocessed data for training and validation.

    Args:
        path (str): The path to the directory containing the satellite images.
        N_FILES (int, optional): The number of satellite images to choose. Defaults to 1000.

    Returns:
        tuple: A tuple containing the preprocessed training and validation data. The tuple has the following elements:
            - train_x (list): A list of training satellite images.
            - train_y (list): A list of corresponding ground truth masks for the training images.
            - val_x (list): A list of validation satellite images.
            - val_y (list): A list of corresponding ground truth masks for the validation images.
    """
...
def get_google_maps_data(path: str, N_FILES=1000):
    """Get Google Maps data for road segmentation.

    This function retrieves satellite images and corresponding ground truth files from the specified path.
    It randomly selects N_FILES number of satellite images and preprocesses the data.

    Args:
        path (str): The path to the directory containing the satellite images and ground truth files.
        N_FILES (int, optional): The number of satellite images to choose. Defaults to 1000.

    Returns:
        tuple: A tuple containing the preprocessed training and validation data.
            - train_x (list): A list of preprocessed training satellite images.
            - train_y (list): A list of corresponding ground truth labels for the training images.
            - val_x (list): A list of preprocessed validation satellite images.
            - val_y (list): A list of corresponding ground truth labels for the validation images.
    """
    # List all files in the directory
    print("Listing files")
...
def get_massa_data(path: str, N_FILES=1000):
    """The Massachussets dataset.
    
    This function takes a path to the directory containing the Massachussets dataset and returns preprocessed data for training and validation.
    

    Args:
        path (str): The path to the dataset directory.
        N_FILES (int, optional): The number of files to choose randomly from the dataset. Defaults to 1000.
...
def preprocess_split_data(images, groundtruths):
    ...
    Args:
        ...
        groundtruths (list): A list of ground truth file paths corresponding to the images.

    Returns:
        tuple: A tuple containing the training and validation data in the following format:
            - (train_x, train_y): Training images and their corresponding ground truths.
            - (val_x, val_y): Validation images and their corresponding ground truths.
    """
    ...
    train_x, val_x, train_y, val_y = train_test_split(images, groundtruths, test_size=0.2, random_state=42)
...
def _load_data(path: str = TRAIN_PATH, val_size: float = 0.2) -> tuple[tuple[list[str], list[str]], tuple[list[str], list[str]]]:
    ...
    assert len(train_x) == len(train_y), f"Number of images and groundtruth images do not match: {len(train_x)} != {len(train_y)}"

    # Check that filenames in train_x and train_y are all the same
    for x, y in zip(train_x, train_y):
        x_filename = os.path.basename(x)
        y_filename = os.path.basename(y)
        assert x_filename == y_filename, f"Filename mismatch: {x_filename} != {y_filename}"
...
def _crop_image_and_mask(img, mask):   
    """Crop the image and mask into smaller regions and return the best crop based on non-black pixel count.
    ...
    Returns:
        tuple: A tuple containing the best cropped image and mask.
    """
...
def _read_image(path: Union[str, bytes]) -> np.ndarray:
    ...
    Args:
        path (Union[str, bytes]): The path to the image file.
...
def _tf_parse(x, y):
    """Parse the input and ground truth images.

    This function takes the input and ground truth images as input and performs the necessary parsing operations.
    It reads the input image and ground truth image using the `_read_image` and `_read_gt` functions respectively.
    The parsed images are then returned as numpy arrays.
    ...
    Returns:
        A tuple containing the parsed input and ground truth images as numpy arrays.
    """
...
def _tf_dataset(x: list[str], y: list[str], batch_size: int = BATCH_SIZE):
    ...
    Args:
        x (list[str]): A list of input data file paths.
        y (list[str]): A list of target data file paths.
        batch_size (int, optional): The batch size for the dataset. Defaults to BATCH_SIZE.

    Returns:
        tf.data.Dataset: A TensorFlow dataset containing the input and target data.
    """
...
def increase_contrast(image, factor=2.0):
    ...
    Args:
        ...
        factor (float, optional): The contrast enhancement factor. Defaults to 2.0.
...
def apply_laplace_filter(image):
    """
    Apply Laplace filter to the input image.
...
def apply_threshold(image, threshold=175):
    ...
    This function converts the input image to grayscale, applies a threshold value,
    and converts it to a binary image.

    Args:
        image (PIL.Image.Image): The input image.
        threshold (int, optional): The threshold value. Pixels above this value will be set to 255 (white),
            and pixels below or equal to this value will be set to 0 (black). Defaults to 175.
...
def detect_shadows(img):
    """
    Detects shadow regions in an image.

    Args:
        img (numpy.ndarray or PIL.Image): The input image.

    Returns:
        numpy.ndarray: A binary mask where shadow regions are white and non-shadow regions are black.
    """
